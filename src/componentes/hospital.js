...
export default function HospitalDashboard() {
  ...
  const fetchAppointments = () => {
    axios
      .get("http://localhost:4000/manipalhospital")
  ...
  const handleAccept = (id) => {
    axios
      .patch(`http://localhost:4000/manipalehospital/${id}`, { status: "Accepted" })
  ...
  const handleReject = (id) => {
    axios
      .patch(`http://localhost:4000/manipalhospital/${id}`, { status: "Rejected" })
      .then(() => fetchAppointments())
      .catch((err) => console.error(err));
  };
