...
export default function HospitalDashboard() {
  ...
  const fetchAppointments = () => {
    axios
      .get("http://localhost:4000/fortishospital")
  ...
  const handleAccept = (id) => {
    axios
      .patch(`http://localhost:4000/fortishospital/${id}`, { status: "Accepted" })
  ...
  const handleReject = (id) => {
    axios
      .patch(`http://localhost:4000/fortishospital/${id}`, { status: "Rejected" })
      .then(() => fetchAppointments())
      .catch((err) => console.error(err));
  };
